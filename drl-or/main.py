...
def main():
    ...
    # 构造 node_num 个 PPO Agent，
    # 每个Agent对应一个Actor-Critic网络，
    # 每个Agent对应一个数据缓存池 rollout 存储多个step采集的数据
    actor_critics = []
    agents = []
    rollouts = []

    # 生成 num_agent 个 actor_critic网络
    # 每个 agent 利用 rollout 存储交互得到的数据
    ...
    # Pre training
    # 一个step，所有智能体做出决策，生成路由路径
    # time_costs = []
    # 预训练30个epoch, 每个epoch 128个step
    # 预训练环境下不与环境交互
    ...
    for i in range(args.num_pretrain_epochs):
        for j in range(args.num_pretrain_steps):
            ...
            # 预训练情况下设置simenv为false，不和ryu与mininet交互
    ...
    rollouts = []
    for i in range(num_agent):
        rollout = RolloutStorage(args.num_steps, observation_spaces[i].shape, action_spaces[i], actor_critic.recurrent_hidden_state_size, num_node)
        rollouts.append(rollout)
        rollouts[i].obs[0].copy_(obses[i])
        rollouts[i].to(device)
